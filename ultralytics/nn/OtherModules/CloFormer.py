...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.spatial_conv = nn.Sequential(
            ...
            nn.Conv2d(dim // 16, 1, 3, padding=1, bias=False),
            nn.Sigmoid()
        )
...
class CloFormerAttnConv(nn.Module):
    ...
    def __init__(self, dim, num_heads=4, group_split=[2, 2], kernel_sizes=[5], window_size=7,
                 attn_drop=0.3, proj_drop=0.3, qkv_bias=True):
        super().__init__()
        ...
        total_channels = sum([g * self.dim_head for g in group_split])  # 实际concat后的总通道数
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(total_channels, total_channels // 16, 1),  # 压缩比为16
            MemoryEfficientSwish(),
    ...
    def forward(self, x: torch.Tensor):
        ...
        spatial_mask = self.spatial_attn(out)
